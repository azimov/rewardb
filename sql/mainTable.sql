...
SELECT fr.TARGET_COHORT_ID, t.TARGET_COHORT_NAME, fr.OUTCOME_COHORT_ID, o.OUTCOME_COHORT_NAME, tc.ATC3_CONCEPT_NAME AS EXPOSURE_CLASS,
    CASE
        WHEN harm_t.sc_harm_count IS NULL THEN 'none'
        WHEN harm_t.sc_harm_count = 1 THEN 'one'
        WHEN harm_t.sc_harm_count >= 4 THEN 'all'
    ...
    CASE
        ...
        WHEN benefit_t.sc_benefit_count >= 4 THEN 'all'
...
FROM results fr
    ...
    LEFT JOIN exposure_classes tc ON fr.TARGET_COHORT_ID = tc.TARGET_COHORT_ID
