...
forestPlot <- function(table) {
    ...
    geom_point(aes(shape = SOURCE_NAME, color = SOURCE_NAME, size = SOURCE_NAME)) + scale_shape_manual(values = c(16,
        16, 16, 16, 16)) + scale_size_manual(values = c(3, 3, 3, 3, 3)) + scale_color_manual(values = c(
        "black", "black", "black", "black", "black")) + geom_text(vjust = 0, nudge_y = 0.2) + geom_errorbarh(height = 0.1) +
...
getMetaAnalysisData <- function(table) {
    ...
    row <- data.frame(SOURCE_ID = -99, SOURCE_NAME = "Meta Analysis",
