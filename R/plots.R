...
computeCalibratedResults <- function(connection, outcomeControlIds) {
  sql <- "SELECT * FROM results WHERE C_PT > 0 AND T_PT > 0"
  dataSet <- DatabaseConnector::renderTranslateQuerySql(connection, sql)
  dataSet$logRr <- log(dataSet$RR)
  dataSet$logRr <- log(dataSet$)

  negatives <- dataSet[dataSet$OUTCOME_COHORT_ID %in% outcomeControlIds, ]
  `%notin%` <- Negate(`%in%`)
  interest <- dataSet[dataSet$OUTCOME_COHORT_ID %notin% outcomeControlIds, ]

  nullDist <- EmpiricalCalibration::fitNull(logRr = negatives$logRr, seLogRr = negatives$seLogRr)
  calinbratedPValue <- EmpiricalCalibration::calibrateP(null, interest$logRr, interest$seLogRr)
  errorModel <- EmpiricalCalibration::convertNullToErrorModel(null)
  ci <- EmpiricalCalibration::calibrateConfidenceInterval(interest$logRr, interest$seLogRr, errorModel)
}