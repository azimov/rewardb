...
serverInstance <- function(input, output, session) {
  ...
  if (tableExists("time_on_treatment")) {
    ...
    forestPlotServer("forestPlot", dbConn, queryDb, appContext, filteredTableSelected)
    calibrationPlotServer("calibrationPlot", dbConn, queryDb, appContext, filteredTableSelected)
}
